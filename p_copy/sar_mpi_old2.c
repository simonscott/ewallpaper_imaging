...
complex* shared_mpi_buf;                  // The packet buffer
...
void* chip_thread(void* threadid)
{
  ...
  complex* mpi_buf = shared_mpi_buf + tid * (Nf + 16);
...
int main(int argc, char* argv[])
{
  ...
  shared_mpi_buf = (complex*)safe_malloc((Nf + 16) * N_thread_x * N_thread_y * sizeof(complex),
                  "Failed to malloc memory for mpi_buf");
  ...
  // Launch the threads
  start_virtual_network(N_thread_y, N_thread_x, chip_thread);
