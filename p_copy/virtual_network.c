...
void init_processor(int i){
  ...
  processor* p = get_processor(i);
  pthread_cond_init(&(p->execution), NULL);
  pthread_mutex_init(&(p->execution_lock), NULL);

  //Inbox
  p->num_messages = 0;
  p->messages = (char**)safe_malloc(max_messages * sizeof(char*), "Could not allocate inbox.");
  p->inbox_top = (char*)safe_malloc(message_memory, "Could not allocate inbox memory.");
  printf("message_memory = %d\n", message_memory);
  printf("allocated inbox top = %ld, top-char = '%c'\n", (long)p->inbox_top,
         p->inbox_top[0]);
}
...
void start_processor(int i, processor_main_function main_function){
  ...
  processor* p = get_processor(i);
  int failure = pthread_create(&(p->thread), NULL, thread_entry, (void*)(long)i);
  if(failure){
...
void add_message(int i, char* message, int size){
  ...
  printf("move message, inbox_top = %ld, message = %ld, size = %d\n",
         (long)p->inbox_top, (long)message, size);
  printf("message = '%c'\n", message[0]);
  printf("inbox_top = '%c'\n", p->inbox_top[0]);
  //memmove(p->inbox_top, message, size);
  printf("increase num messages\n");
...
void send_virtual_message(int i, char* message, int size){
  printf("send_virtual_message\n");
  processor* p = get_processor(i);
  printf("got processor\n");
  pthread_mutex_lock(&(p->execution_lock));
  printf("got lock\n");
  add_message(i, message, size);
  printf("added message\n");
  pthread_cond_signal(&(p->execution));
  printf("signaled processor\n");
  pthread_mutex_unlock(&(p->execution_lock));
...
char* receive_virtual_message(int threadid){
  processor* p = get_processor(threadid);
  if(p->num_messages == 0)
    wait_for_messages(p);
  return p->messages[0];
}
