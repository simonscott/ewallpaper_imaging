...
void* processor_main(int MYTHREAD){
  ...
  if(MYTHREAD == 0){
    int buffer[] = {0};
    send_virtual_message(1, (char*)buffer, sizeof(int));
    
    while(1) {
      // Receive message
      int* message = (int*)receive_virtual_message(MYTHREAD);
      buffer[0] = *message;
      free_virtual_message(MYTHREAD, (char*)message);
      // Process Message
  ...
  if(MYTHREAD == 1){
    ...
    while(1){
      ...
      // Send to Destination
      buffer[0] += 1;
      send_virtual_message(0, (char*)buffer, sizeof(int));
      if(buffer[0] >= N)
        break;
    }
  }
